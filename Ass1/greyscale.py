...
def grey_one(image):
	...
	for i in range(512):
		for j in range(512):
			...
			#new_value = int((image[i][j][0] + image[i][j][1] + image[i][j][2]))/3
			new_image[i][j] = new_value
			#new_image[i][j][1] = new_value
			# new_image[i][j][2] = new_value
			if new_value < 0:
				print("hh")
	return new_image	
...
def grey_two(image):
	...
	for i in range(512):
		for j in range(512):
			new_image[i][j] = 0.2126*image[i][j][0] + 0.7152*image[i][j][1] + 0.0722*image[i][j][2]
